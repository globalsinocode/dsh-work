...
| MCP Connector 治理与执行 | [PostgresToolConnectorService](../../server/src/modules/tool/postgres-tool-connector-service.ts)、[AgentRuntimePort](../../server/src/modules/runtime/runtime-types.ts)与[DSH Adapter](../../server/src/modules/runtime/dsh-acp-runtime-adapter.ts) | 复用 Connector 管理；Streamable HTTP Server 整体发现并自动生效，Agent→Connector 二元 Grant；Attempt 固定能力摘要并复核当前授权；凭据值只进入 Worker 环境，实际调用由 DSH 执行并按 MCP Tool 留审计；不创建平台 Tool Version/Binding |
...
`GET /tools`、`GET /tools/catalog` 与 `GET /tools/bindings` 是 DSH 内置工具的管理边界，只返回稳定 Connector `connector-dsh-workspace` 下的记录。dsh-work 内置执行/平台工具由代码契约目录管理；MCP 外部工具由 Connector 的自动生效发现快照和 Agent Grant 管理，二者均不写入普通工具列表。

`GET /connectors` 是 MCP 管理边界，只返回外部 MCP Connector。平台稳定连接 `connector-dsh-workspace` 通过 `GET /runtimes/dsh-tool-connector` 在 Runtime 运维详情中只读展示，检查使用 `POST /runtimes/dsh-tool-connector/check`；不得从 MCP 页面为其提供新增、Token、Agent Grant 或删除操作。
