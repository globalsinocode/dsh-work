...
| 受控记忆与经验 | [PostgresControlledMemoryService](../../server/src/modules/memory/postgres-controlled-memory-service.ts)、[RunOrchestrationService](../../server/src/modules/run/run-orchestration-service.ts)与[DSH Adapter](../../server/src/modules/runtime/dsh-acp-runtime-adapter.ts) | Agent Version 显式引用 `propose_memory@1.0.0` 后，DSH 只能暂存带来源的 7 天候选；发布试运行返回 `trial_only`、不落库。员工从本人成功 Attempt 核对提案或自行填写，再选择使用范围和期限；提交幂等摘要固定来源、内容、同意参数和提案 ID，同键并发串行。管理员审核后创建独立不可变版本。新 Attempt 按当前用户、空间、来源空间当前授权、当前角色与快照角色的交集及稳定 Agent 身份检索；“仅本人”记忆只进入本人的个人空间。Manifest 固定版本与摘要，Adapter 生成本次只读 `workspace/memory.md` 并标记为非权威参考。审核、领取、运行与等待恢复复核来源授权、版本和同意；撤回、来源撤权或到期阻止未来使用，管理查询和审核重放不再返回正文，历史摘要、审核和实际引用按内容保留规则继续可追溯。 |
