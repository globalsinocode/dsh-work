...
- D12：员工主导航为新对话、历史对话、我的文件、团队空间。后台默认个人 Workspace 保留，员工端不显示它的卡片、选择器和成员管理。全局请求不带 workspaceId，由服务端按当前用户解析；显式错误 ID 必须拒绝，不回退到第一个团队。团队入口固定团队归属；旧 Session/Run 不迁移空间。
