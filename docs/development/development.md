...
PF-03 MCP Connector 治理使用 `pnpm test:mcp:integration`，在一次性数据库中验证平台生成标识、Bearer Token 加密存储/运行时解析/轮换、整体发现与自动生效、Agent→Connector Grant、已准备 Attempt 的能力摘要漂移、逐调用审计和撤权；该命令使用合成 Runtime，不替代真实 Streamable HTTP MCP 与目标 DSH 的 P2 验收。
...
- MCP 以整个 Connector 为 Agent 授权单元；成功检查后的清单自动生效，旧摘要 Attempt、停用或撤权连接在后续调用复核时拒绝，实际 Tool 调用可追溯且凭据不进入 Manifest/Patch。
...
本轮安全修复的范围、兼容约定和单独回归入口见 [批次 0/1](review-batch01.md)。
