...
| AS-04 状态与上下文 | **员工记忆功能已移除；Agent 经验迭代第一阶段已接通。** [Manifest 编译器](../../server/src/modules/runtime/manifest-compiler.ts)、[经验服务](../../server/src/modules/memory/postgres-controlled-memory-service.ts)和 Adapter 复用只读经验投影；Workbench 不提供任何记忆 API，管理端按稳定 Agent 查看成功 Attempt 的申请并审核发布 | 一次性 PostgreSQL 验证 Agent 聚合、来源 Run/Attempt、审核和不可变版本；组件/契约/P0 验证新入口。尚无专属质量评测、显式回滚及真实 DSH/OIDC 证据 | 补经验质量评测、停用/回滚和 P2；业务主数据、文档原文、会话历史和检查点不得写入经验 |
...
| AS-11 版本追溯 | **部分实现**。[迁移 0039](../../server/migrations/0039_agent_release_governance.sql)保存包、候选、试运行和证据；试运行通过后单次审核确认即发布，事务内复核 sealedRevision、当前定义、依赖/绑定及最新 v1 试运行；既有 `submitted` 候选仍可完成或退回；RunAttemptRecord 保存 Manifest 摘要和模型路由快照 | 发布集成覆盖草稿直接发布、修改候选使证据失效、人工判失败阻塞、旧 v1 证据拒绝、既有待审候选完成和绑定漂移；P0 浏览器验证单次确认交互；跨环境变更验证未覆盖 | 与 AS-01 一起补真实发布依赖；小范围启用和目标环境验证仍是发布门槛 |
...
下文按日期保留的实施记录中如出现 `submitForReview`，描述的是当时的两步流程；当前 Agent 发布入口已移除该提交步骤，以本节 AS-11 和[生命周期模板](agent-lifecycle-template.md)为准。
...
| PF-05＋AE-04（员工流程退役；经验迭代第一阶段完成，2026-09-23） | Agent 经验申请与不可变版本 | 员工端界面、客户端类型及 Workbench API 已移除；Agent 从 Run/Attempt 提出申请，管理端按 Agent 审核并发布不可变经验版本 | 组件、契约、P0 与一次性 PostgreSQL覆盖当前范围；专属评测、回滚和 P2 仍未完成 |
...
- **前端与契约：** ConversationView 消费 `task.result.*`；身份行与「对话详情 → 结果核验」区分列展示业务 outcome、回执、待处理项与遥测证据；普通问答及外部业务结果为 `unverified` 时不在消息流展示警示横幅，详细缺口由用户按需在对话详情查看，且不出现「目标已达成」。AutomationsView 执行记录分列受理/执行/业务结果。StatusTag 增加 outcome 与回执状态标签。[openapi-workbench.json](openapi-workbench.json) 新增结果路径与 `TaskResult` 系列 schema；Prototype 夹具提供四种 outcome 样例。
...
**当前状态（2026-09-23 调整）：** 原 PF-05 的员工提交、提案读取、授权查询与撤回产品入口已完整移除，不保留历史数据兼容流程。Agent 经验迭代第一阶段复用既有治理存储：显式获准的 Agent 从 Run/Attempt 提出短期经验申请，只有成功 Attempt 的申请按稳定 `agent_id` 进入“经验迭代”管理页；管理员核对 Agent Version、Run、Attempt 后批准或拒绝，批准发布不可变经验版本。Workbench 没有写入或授权 API。

当前经验内容仍只能作为非权威参考；涉及 Soul、Skill、脚本、Tool、权限或其他 Agent 定义的变化必须创建新 Agent Version 并重新评测发布。下一步补专属质量评测、显式停用/回滚和真实 DSH/OIDC 验收；在这些门禁完成前不得宣称 Agent 自动学习或完整自我迭代。
