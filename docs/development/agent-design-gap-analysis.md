...
**核对日期：** 2026-09-19<br>
**代码基线：** `3f4f4bf`；首次核对开始时工作树干净；I-01/I-02 实施改动与文档更新保留在工作树中，未提交。<br>
...
| AS-10 真实结果 | **部分实现**。编排 `persistEvent` 将 `run.completed` 转成 succeeded；Adapter 收集成果并提交回答；内容服务管理版本与权限。没有独立的通用“目标达成/动作未知”结果外层 | Adapter 覆盖成果收集失败/撤权；Run 与文件测试验证执行及成果机制，不能证明所有业务目标达成 | 设计结果外层及消费者兼容；保留 Run 执行状态，将业务完成、回执和未解决事项独立表达 |
...
当前 `parseAgentPackage` 要求 id/name/version/description 与有效 Prompt，支持内联或文件形式；无 `evals/cases.yaml` 时发布服务生成默认案例（带 `origin` 来源标记）。缺少 checksums 时解析给警告，发布检查阻塞；提供清单时须覆盖全部文件（除清单自身）。解析、候选检查与发布是不同阶段，当前不支持用“平台自动生成摘要”替代 ZIP 发布门禁。

**2026-09-19 更新：** 字段政策已由 I-02 落地并验证。扁平字段白名单（含别名）固化在解析器头部注释；同一字段别名并存按“取值一致警告、不一致拒绝”处理（列表按元素集合比较，顺序不视为冲突）；`system_prompt` 与 `system_prompt_file` 并存按内容一致性同样处理；`apiVersion/spec` 结构清单给出明确格式不支持错误；身份、凭据、端点、模型路由、执行环境、外部能力接入与安装钩子等平台受管字段直接拒绝；其余未识别字段警告后忽略。依赖引用校验 `id`/`id@x.y.z`：空条目、残缺引用、同 id 多版本均拒绝；声明版本与包内候选版本冲突由“警告并覆盖”改为明确拒绝。`skills/`、`tools/` 目录缺少 SKILL.md/tool.yaml 描述符时警告提示。tool.yaml 内 `id` 与 `name` 为不同字段不作别名处理，但其取值须符合能力标识规则。YAML 对象化错误（含锚点/别名禁用）归入统一 422 校验错误。旧式扁平清单（含别名与内联 Prompt）有兼容用例。未来格式升级（apiVersion/spec、结构化输入输出等）仍须单独制定版本识别、旧包兼容和迁移规则，不在本项范围。
...
当前 succeeded 表示平台执行终态，不能直接作为业务完成标志。后续结果外层应关联已有 Run、消息、来源和 Artifact，而非另建执行状态机；先定义读写消费者及失败语义，再决定字段和迁移。
...
下列工作项将差异转为具体交付内容；完成状态以各工作项的完成记录为准（当前 I-01、I-02 已完成）。I-01～I-08 是建议近期推进的修复、基础建设及证据补齐；I-09～I-13 有明确需求后才启动。未标完成的新字段、表与接口均为待设计内容，不表示当前已经支持。工程归属沿用各阶段实施文档（如 [AG-03 实施方案](../design/automation-implementation-plan.md)）及 [Runtime EX 方案](../design/runtime-execution-optimization-plan.md)，此处维护差异对应的工作分解。

为兼容已有代码和验收目录，保留阶段编号：AG-01 表示 Agent 包与发布，AG-02 表示工具扩展，AG-03 表示轻量自动任务，AG-04 表示受控经验。AG-01/02/04 的后续工作直接以本文 I 编号为入口，无需查阅已删除的总方案；AG-03 与 EX 的细节使用上述现存文档。阶段编号不表示能力已实现或已授权启动。
...
| I-02 明确包校验与兼容 | AS-02/11，D-02 | ~~字段规则、依赖冲突拒绝、旧包兼容用例和校验反馈~~ **已完成（2026-09-19）** | 可独立；AG-01 包与发布 |
...
| I-13 无 Session 任务入口 | AS-01/10，D-04 | 独立触发来源、任务结果读取与访问授权、兼容迁移 | 按需；I-03/04/06/08；保持既有对话及自动任务入口 |

建议先交付 I-01、I-02 和 I-07 的既有限额核查；I-08 的现有链路验证可以同时开展。I-03/04 明确契约后推进 I-05/06，不因可选扩展推迟基础修复。这里的先后次序不改变 P0/P1/P2 验证层级含义。
...
1. 盘点内联与外置 Skill 的实际生成数据，明确两种合法形式及是否兼容带内联缓存的外置引用；不通过去掉 `additionalProperties: false` 放宽整个对象。
...
1. 列出当前扁平清单允许的字段、别名、默认值和长度/类型限制；明确未知字段、未支持版本、受管身份/凭据/端点声明的错误规则。针对历史包先提供检查反馈和迁移说明，再收紧导入；不能静默忽略可能影响安全或能力语义的字段。
...
4. 补最小有效包、依赖冲突包、安全字段误用包和旧包兼容样本；更新管理 API 错误与导入反馈。是否升级为 `apiVersion/spec` 单独决策，不能以本项名义强制重打所有旧包。

**完成标准：** 受支持旧包在新校验下行为明确；冲突在入库/发布前被识别，不因重名替换依赖；同来源版本异内容仍拒绝。解析单测、发布集成及受影响导入旅程通过。
...
4. 定义模型/Runtime **能力要求**及兼容判断；模型路由继续由平台选择。无兼容路由/Runtime 时拒绝准备，不从包读取 Provider 凭据，也不建立备用模型循环。已发布旧版缺省值固定为原行为，不能随新默认配置静默变化。
5. 同步持久化、公开契约与管理端只读展示；新字段改变执行语义时推进候选修订，使旧验证证据失效。迁移不得覆盖既有发布定义。

**完成标准：** 相同配置和 ZIP 生成等价规范化定义；旧版保持原行为；无效输入、来源撤回和能力不兼容均有明确拒绝；修改执行字段后必须重新验证。用既有 Agent 完成配置→发布→运行回归。
...
4. Attempt 固定实际绑定引用及解析证据，执行时另查当前权限和撤销状态。旧 Session/自动任务继续引用原发布版本，旧绑定不可用时明确失败，不静默改用新连接。
5. 制定已有发布版本的迁移规则：保留可证明的旧配置；不能还原的历史标记为未记录，而不是伪造绑定摘要。新增运行前补齐必要验证。

**完成标准：** 同定义在两个批准环境可独立发布；换端点/身份后旧证据不能放行；并发改绑定时发布安全失败；历史运行可查原依据，撤权仍即时受执行边界约束。通过迁移、发布并发、回滚和权限集成验证。
...
1. 盘点当前批准工具，建立共用契约：精确版本、输入/输出 Schema、动作性质、资源范围、超时/取消、审批、重试及并发要求。目录、模型可见描述、实际校验和结果投影使用同一版本；对仍不具备结构化输出的工具明确能力边界，不填空 Schema 后宣称完整校验。
...
4. API 采用可兼容的增量返回；旧 Run 无结构化结果时展示旧记录及证据缺失，不凭历史文本自动补“已验证”。前端使用既有组件展示结果、回执和待处理事项，动作提交重新鉴权。
...
3. 保留现有存量配置兼容，明确 `maxTokens * 4` 的估算用途；不要无迁移地将它改成成本或模型 Token 承诺。收紧上限需进入候选/任务配置确认及 Manifest 冻结流程。
...
**完成标准：** 对每条门槛能找到对应测试或人工验收结果，注明环境、版本、运行时间和失败/未覆盖项；用例存在、P1 通过与 P2 通过分别报告。此项既验证当前实现，也为 I-01～07 提供持续回归门禁。
...
**改动范围与依赖：** Run、Runtime 契约、HTTP 接入、内容服务及数据库；依赖 I-03/04/06/08。现有员工/管理对话和 AG-03 独立 Session 路径保留兼容。
...
验收记录至少包含工作项编号、代码版本、改动范围、兼容/迁移结果、实际运行的测试及环境、未覆盖项和真实验收状态。已通过检查只在后续变更影响它时重跑；只有相关证据齐备才将工作项改为完成。新增外部写入、身份类型或执行引擎能力超出已确认范围时，先完成具体设计评审。
...
未运行浏览器 E2E 或真实 DSH/OIDC 验收；表中的测试链接用于指出现有覆盖及后续入口，不声明本清单外条目已通过。未提交、推送或发布。
