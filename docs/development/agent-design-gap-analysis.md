...
| PF-03（代码级与 P1 已完成，2026-09-22） | MCP 受控接入 | 复用 Connector 的 Streamable HTTP MCP Server 登记、添加前不落库连通测试与服务端复核、平台生成内部标识、Bearer Token 加密入库与轮换、完整 Tool 清单发现、Connector 整体审核、Agent→Connector 二元 Grant、能力摘要漂移门禁、受控删除、DSH 调用和 Tool 级审计；不生成平台 Tool Version/Binding | P1 服务集成与管理端浏览器旅程完成；真实批准服务的发现→审核→授权→调用→撤权、真实凭据和目标 DSH 证据留 PF-07 P2；Resources/Prompts/stdio 不在当前支持范围 |
