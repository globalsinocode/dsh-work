...
**代码基线：** 初次核对为 `3f4f4bf`；当前已提交基线为 `12435b0`，叠加通用 Agent 全生命周期模板的未提交文档改动。B-01～B-04 已落地；B-05 代码级基础已完成，P2 已由用户确认为手工执行完成，仓库内审计证据仍待按通用模板归档。完成记录与剩余范围见第 4、5 节。<br>
...
**下一步：按[通用 Agent 全生命周期模板](agent-lifecycle-template.md)推进所有新 Agent。** 先将已手工执行的 P2 补录为可审计验收包；随后每个 Agent 均按工作定义、能力分解、扩展判定、AgentSpec、绑定、五类评测、DSH 试运行、发布、结果核验和演进/退役推进。业务场景只填写模板，不新增场景专属平台流程。I-09～I-13 仅在模板触发条件成立时启动，不因清单顺序自动实施。
...
**2026-09-21 通用 Agent 全生命周期模板（当前未提交工作区）：**
