...
当前不需要为第 1～3 阶段新增数据库状态，也不在 AgentSpec 中增加 MCP、持久化等待、记忆、委派和无 Session 入口的空开关。只有扩展触发条件成立时，才设计对应契约、持久化和验收。
...
| 5. 能力准入与绑定 | 已发布 Skill/Tool Version、`tool_binding_revisions`、版本 `binding_refs` | Agent、Skill、Tool 服务与 Runtime 快照 | 当前状态、精确版本、真实绑定修订、摘要、身份和数据范围复核 | **已实现基础链路**。多环境独立发布仍需具体 Agent/P2 验证 |
...
- MCP Host/Client、Server 发现和远端能力绑定；
- 跨 Attempt 的持久化等待状态机；
- 跨 Session 受控记忆；
- Agent 委派及父子预算；
- 无 Session 的新执行入口；
...
已有 P2 被报告为手工执行，但环境、版本、Run/Attempt、验收人和限制尚未写入仓库。在这些字段归档前，其仓库状态仍是“已执行，证据待归档”。
