...
| 交付 | 包含 | 独立验收边界 |
| --- | --- | --- |
| AG-01：Agent 管理首版 | 配置创建、ZIP 导入、已有 Skill/Tool 引用、支持格式的包内 Skill、真实 Agent 试运行、负责人发布、共享目录及完整生命周期 | 至少一个使用现有工具的业务能力由另一员工按自身权限使用；不等待新执行器、定时或经验模块 |
...
AG-00 先冻结最小契约与业务案例，详见 §15。AG-01 上线前不开放尚未完成的 Tool 候选执行入口；对包含新 Tool 的包明确显示所缺能力。已有工具的 Agent 可以独立交付，包含新工具的 Agent 必须等其工具适配与证据完成。
...
| 阶段 | 主要工作 | 独立退出条件 |
| --- | --- | --- |
| AG-00：业务案例与最小契约 | 确定首个使用者与场景；冻结配置/包归一化、能力命名、定义/绑定/发布映射、共用准备端口、证据与启停语义；准备有效/无效样包 | 业务结果可观察，逻辑依赖能映射到精确版本，字段归属与关键拒绝场景明确；不要求 CLI 或新增执行器 |
| AG-01：Agent 管理首版 | 配置/ZIP 共用候选、已有能力与包内 Skill、文件存储引用、共用运行准备、真实试运行、负责人发布与目录、升级/回滚/启停/卸载、旧入口一致门禁 | 至少一个使用已有工具的真实业务 Agent 可由另一员工使用；草稿不影响发布版，真实证据与生命周期完整；不等待定时和经验 |
...
1. 选定一个仅需已有工具的真实业务场景及负责人，准备合成案例和真实验收口径。
2. 按当前 Agent/Skill/Tool 表与端口完成字段映射，确定来源版本、平台版本和绑定 revision 的落点；预先检查历史数据和回退兼容。
3. 固化最小 Agent Schema、逻辑能力解析、两种创建入口的归一化与拒绝样例；不提前开放未适配工具。
4. 在设计中走通创建、编辑草稿、试运行、发布、普通停用、撤销和卸载的状态与权限；确认原型后按项目规则实施关键旅程。
5. 将 AG-01 拆为可验证工作项，绑定 §16 的验收编号与真实 DSH 证据要求；不同时启动本方案所有后续模块。
...
- P2 真实验收：记录目标 DSH Lock、Adapter、模型、批准企业 Connector 与数据，验证真实调用、来源、输出、成果及取消；真实 OIDC、多账号收权和 macOS 环境分别留证。Mock、单一探针和页面成功不能互相替代这些证据。
...
| 交付 | 必须达到的完成条件 |
| --- | --- |
| AG-01 Agent 首版 | 最小规范和两种创建入口可用；真实试运行/业务确认/发布及旧入口门禁一致；定义、绑定、快照与生命周期规则成立；不同员工用同一已有工具 Agent 获得可核验结果；文件存储及历史保留有效 |
