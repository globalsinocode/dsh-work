...
**状态：** 分阶段实施中。AG-00 契约已按调整范围收口；AG-01 管理端主线已交付代码并通过 P1 集成验证（候选/检查/真实 DSH 试运行/逐项确认/提交-退回-撤回/事务内发布/版本证据），管理端 API 为当前实现基线；员工自助入口、紧急撤销与卸载、AG-02 工具扩展、AG-03 自动任务、AG-04 经验积累尚未实施。当前实施进度见 §17.1；本文其余部分仍是目标设计，不代表全部能力已实现。<br>
**更新日期：** 2026-09-15<br>
...
| AG-01：Agent 管理首版 | 配置/ZIP 共用候选、已有能力与包内 Skill、文件存储引用、真实试运行、负责人发布与目录、升级/回滚/启停/卸载、旧入口一致门禁 | 至少一个使用已有工具的真实业务 Agent 可由另一员工使用；草稿不影响发布版，真实证据与生命周期完整；不等待定时和经验 |
...
> - **真实业务场景与验收口径**：场景选定与真实验收口径是 AG-01 退出验收（P2）的输入，随验收执行确定，不再作为契约冻结的前置工作单列；
...
| 阶段 | 实施状态 | 已交付 | 未交付 |
| --- | --- | --- | --- |
| AG-00 契约冻结 | **已收口（范围经评审调整，见 §15.2）** | 最小包规范与归一化、依赖精确解析、提交/启停/证据语义冻结、有效/无效样包（集成测试夹具） | 能力命名域、共用运行准备端口、绑定修订落点移出本期；真实业务场景与验收口径归入 AG-01 的 P2 验收输入 |
