...
| AG-03：定时自动任务 | 本人身份、固定版本、每日/每周、立即运行、暂停、取消、去重与失败留痕；不补跑、不自动重试 | 独立完成自动任务验收；可以使用 AG-01 的已有工具 Agent，不依赖经验模块 |
...
本节是 AG-03 的独立交付范围，可在仅使用已有工具的 Agent 上实现，不是 AG-01 发布条件。首版定位为**按计划发起普通员工任务**，参考 Octop 的轻量调度职责划分，继续使用既有 Run/Attempt → Runtime Adapter → DSH。实施契约见 [AG-03 轻量自动任务实施方案](automation-implementation-plan.md)。

**2026-09-17 范围调整：** 首版采用单进程调度，不补跑停机遗漏、不自动重试、不续办中断的准备阶段、不建设独立投递状态机或外部通知系统；任务试运行改为可选，预算采用平台默认限制。允许因服务中断漏一次执行，但遗漏、中断和结果持久化失败必须可见。固定版本、本人身份、授权上限、最小触发去重、原子受理和交互容量保护继续保留。

### 9.1 从已发布能力创建

员工在智能体详情或已完成任务中选择“设为定时任务”，检查输入、数据范围、结果归属、日历和平台默认限制后明确启用。首版只提供结构化每日/每周/手动选项及 IANA 时区，自然语言频率解析移出首版。

| 项目 | 首版规则 |
| --- | --- |
| 执行身份 | 创建者本人，服务端建立；不支持服务主体或代他人授权 |
| Agent 版本 | 固定具备有效发布证据的 Agent 平台版本及精确依赖；不支持 `active_at_trigger` |
| 能力范围 | 仅已批准、无需人工工具审批且具有当前授权与资源约束执行能力的工具；不能在无人值守时等待尚未接通的审批 |
| 输入 | 确定性任务模板、已授权且固定版本的文件或受控查询绑定；动态查询窗口按计划槽固定 |
| Session | 每次新执行新建独立 Session，不带入历史；同一触发重放复用原对象 |
| 触发 | 每日、每周和手动立即运行；预览与调度共用日历算法 |
| 并发 | 每任务最多一个未终结执行；重叠跳过并记录原因，不无限排队 |
| 停机遗漏 | 不补跑；记录遗漏区间，恢复后安排下一个未来槽位 |
| 结果 | 复用指定个人或团队 Workspace 的 Session 消息和成果及其权限；无外部业务回写、无独立投递系统 |
| 经验 | AG-04 未交付前不开放经验选项；后续仅使用已审核、已授权版本 |

任务级试运行可选，用于启用前预览结果；不再强制维护“每个任务配置 revision 必须先试运行成功”的门禁。选择试运行时仍走同一 DSH 链路并展示真实结果，失败不能标为通过。免于任务级试运行不免除 Agent 发布证据和确定性配置、权限、依赖、审批及输入检查。

修改执行版本、输入、授权范围或归属后需重新检查并由本人确认；未确认的执行面修改不能继续自动触发。只改频率/时区时验证日历和当前权限，不重新发布 Agent。暂停、立即运行和取消均是明确操作，不由模型自行决定。

### 9.2 触发去重与原子受理

首版单进程调度，部署和启动必须保证只有一个调度所有者；不支持多实例同时调度，不引入跨阶段领取租约及自动接管协议。数据库唯一键仍保护同槽重复回调和手动请求重放，不把它表述为工具效果“恰好一次”。

1. 在任务级短事务中读取 enabled 状态、schedule revision 和槽位，冻结执行配置及输入引用；复核本次操作仍适用。手动请求另按当前配置和本人授权受理。
2. 按 `(tenant_id, automation_id, trigger_id)` 去重。重复请求返回原记录；手动同键异输入拒绝。
3. 在任务锁内根据已有受理记录及其 Run 的当前状态判断重叠；跳过也持久化记录，不能用唯一索引冲突代替跳过事实。
4. 通过可组合事务端口原子保存触发记录、Session、Run 及关联，推进已处理槽位；不允许只创建新 Session 后依靠会话内 Run 幂等键去重。身份和对象检查在对应准入边界完成，事务内不调用外部服务或 DSH。
5. 事务提交后在 Run 模块复用统一准备，提交不可变 Attempt 并进入现有调度器。重复受理不能再次调用准备；准备失败明确收敛，崩溃后的处理见 §9.4。

触发记录只保存触发、配置快照、关联及受理/跳过/中断事实，执行状态以 Run/Attempt 为准，不复制运行状态机。不存在独立的 `claimed → linked → dispatched → running` 自动化状态流。

定时触发标识由任务、schedule revision 和计划 UTC 槽确定，不使用领取时间。手动立即运行使用请求幂等键；新的“再次运行”操作使用新键并创建新执行。夏令时缺失本地时刻跳过，重复时刻取第一次；预览和执行共用规则。游标更新与槽位留痕原子提交，首次启用、修改规则、重新启用及服务恢复均从下一个未来槽开始，不追补旧规则或暂停期间的槽位。

### 9.3 当前权限与撤销

- 创建、启用、触发受理、实际执行、每次外部工具调用和结果持久化/读取分别重新检查对应当前权限。
- 有效授权为当前授权与启用时批准上限的交集；用户后来获权不自动扩大旧任务范围。交集参与 Agent/Tool 准入、知识检索、Manifest 及工具执行；不能复用现有取并集的 `dataScopes` 入参表达上限。
- 执行前检查覆盖个人和团队空间，并比较已持久化 Manifest 与当前许可。范围收紧导致旧输入或快照越权时拒绝该 Attempt，不原地改写快照，也不继续执行旧权限；用户重新发起时重新授权。
- 账号停用、成员移除、角色降级、Workspace 归档或能力紧急撤销后，阻止新执行并收敛排队及活动任务。活动检查须包含 Workspace、能力与数据范围，不能只校验 subject 的 active 状态。复用并补齐现有授权、撤权和工具边界，不另建执行循环。
- Agent/Tool 普通停用按 §6.5 停止新触发、暂停相关自动任务并清理队列，已运行且仍获授权的任务允许排空；与紧急撤销区别处理。
- 后台身份不持有浏览器 Cookie 或长期 OIDC Token，不伪造 `RequestIdentity`；通过目录同步状态和明确新鲜度策略判断主体有效性，无法确认则拒绝执行并留证。生产不得回退 Prototype。
- 暂停同时停止新触发和已受理但未开始执行的任务；活动执行仅在明确取消或安全撤权时终止。重新启用不复活旧 Run。

### 9.4 失败、容量与结果

- 首版不自动重试。准备失败或服务重启时仍无 Attempt 的已受理 Run 收敛为失败/中断，不续办准备，不重新分配 Session/Run。
- 已提交且仍为当前 queued 的 Attempt 复用现有 Run 恢复并重新鉴权；活动 Worker 丢失时失败收敛。不得因自动化触发记录未更新而再建 Attempt 或启动终态 Attempt。
- 用户可明确“再次立即运行”：这是新手动触发、新 Session/Run，重新检查当前配置和权限。同一请求重放仍返回原执行。自动任务首版不提供对原 Run 的自动重试；任何通用重试入口未接入同等上限、暂停及重叠检查前不得绕过该约束。
- 首版采用平台默认超时、工具调用、输出和所用执行器已有资源限制，以及简单的每用户/全局待处理上限和自动任务并发限额；不新增用户可配置的 token/磁盘预算体系，不把输出字节上限称为模型 token 或磁盘硬预算。
- 自动任务并发限额与 Runtime 总容量联动，保留交互余量；容量不足时自动任务等待或按明确规则跳过。限额计数与领取原子完成，受限自动任务不得堵住队首后的交互任务，取消尚未释放的 Worker 仍计入占用。
- 结果复用既有 Session 消息、成果收集及对象权限。Session 存在不代表结果成功；必要结果持久化失败必须展示明确失败，不能标成已交付。完成的 Run 不因刷新或展示失败重新调用 DSH；既有事件投影恢复继续按其契约执行，AG-03 不新增投递状态机和重试系统。
- 列表展示固定版本、时区、下次运行、最近执行、跳过/遗漏/中断原因以及 Run 结果链接。输入和结果正文按业务对象权限读取，不进入治理概览。
...
| `agent_automations` | Automation | 本人身份、固定 Agent Version、Workspace、日历/时区、输入引用、授权上限、默认限制政策引用和 revision |
| `automation_executions` | Automation | 唯一触发标识、计划槽或遗漏区间、冻结配置/输入、Session/Run 关联、受理/跳过/中断事实；运行结果读取 Run |
...
6. 自动触发唯一键为 `(tenant_id, automation_id, trigger_id)`，执行到 Session/Run 的关联唯一且原子提交，准备中断收敛失败；不能只依靠会话内 Run 幂等键。
7. Run/Attempt 终态归现有运行模块，自动任务、测试、经验和发布协调不得另建 Agent 运行状态机；通用 Run 重试新建 Attempt，AG-03 首版的“再次运行”按 §9.4 创建新的手动触发。
...
| 拟增 `modules/automation` | 日历、幂等触发、后台身份复核、简单限额、遗漏/中断留痕与 Run 结果关联 | Agent 配置和第二套运行状态机 |
...
| 后台主体与事务受理 | 自动任务按本人当前资格执行；幂等且原子创建 Session/Run 及关联；准备中断失败收敛，已提交 Attempt 复用 Run 恢复 |
...
| AG-03 / Workbench | `/api/workbench/v1/automations`，以及 `trial-runs`、`enable`、`pause`、`run-now`、`executions` | 仅本人任务；日历与输入修改绑定 revision，试运行可选，执行取消复用 Run 授权端口 |
...
| 我的自动任务 | 结构化频率与确认、固定版本/本人身份、下一次运行、Run 结果、遗漏/失败、暂停和取消；与 Agent 发布独立 |
...
部署审核、经验审核及结果持久化状态是平台业务事实，可以投影为对话消息，但内容必须来自持久化结果，不能从模型自报推断成功，也不能当成 ACP 工具审批。已有审批事件不代表首版提供等待人工工具批准后恢复执行的能力。
...
| 结果持久化或展示失败 | 标明实际失败阶段，不标为已交付，不因此重新执行 DSH；AG-03 不新增独立投递系统 |

### 14.3 重启与故障恢复

- 未完成上传和校验的提交不进入发布状态；文件与数据库引用按原子保存及回收规则恢复。
- AG-03 受理记录与 Session/Run 关联原子提交；重复触发不得再创建 Session/Run。服务停机遗漏仅留痕，不补跑。
- AG-03 已有 Run 但未提交 Attempt 的准备中断收敛为失败；不续办准备，也不能永久停留 queued。
- 排队 Attempt 复用现有恢复链路并重新授权；活动 Attempt 因 Worker 丢失收敛为失败。其他场景的获准重试创建新 Attempt；AG-03 首版不自动重试，用户再次运行按 §9.4 处理。
...
- 已发布经验不因提炼服务重启重复发布；已完成 Run 不因结果展示或读取失败而重复执行。
...
| AG-03：定时自动任务 | 本人身份、固定版本、独立 Session、单进程调度、稳定触发、事务受理、默认限制、暂停/取消、撤权、遗漏留痕与 Run 结果 | 同触发唯一 Run；无自动补跑/重试/准备续办，权限、失败收敛和交互容量保护有证据，可单独上线 |
...
| AC-20 / AG-03 | 重复槽位/手动请求，原子受理及准备阶段重启 | 同触发唯一记录与 Session/Run；无 Attempt 的准备中断失败，已有 queued Attempt 按 Run 恢复，不重复执行 |
| AC-21 / AG-03 | 时区/DST、停机、规则修改、立即运行及再次运行 | 预览与槽位一致；停机/暂停遗漏留痕且不补跑；不自动重试，再次运行是新手动触发，同请求重放去重 |
| AC-22 / AG-03 | 账号停用、成员移除、个人空间撤权、能力撤销 | 新触发/领取被拒绝，活动执行收敛，后台不能沿用旧身份快照 |
| AC-23 / AG-03 | 可选试运行、两次执行间升级 Agent、暂停与重新启用 | 未试运行也可经确定性门禁和本人确认启用；Session 隔离且版本固定，暂停清理未开始任务，重新启用不补旧槽 |
| AC-24 / AG-03 | 结果持久化/展示失败，自动任务集中到期 | 结果可读有据、失败可见，展示失败不重跑 DSH；默认限制生效，受限队首不阻塞交互且保留容量 |
...
| AG-03 自动任务 | 固定发布版、本人授权和独立 Session 成立；触发唯一、原子受理、准备中断失败、遗漏不补跑、默认限制、暂停/取消、撤权和结果持久化分别有证据 |
...
| AG-03 自动任务 | **轻量方案已修订，代码未实施** | 本文 §9 与独立实施稿已同步首版范围，AC-20～24 已登记计划旅程 | 两表、单进程触发、事务受理、权限补齐、暂停/取消/限额、结果关联及各层验证均待实现 |
