...
| 任务与外部操作 | `tasks`、`task_operations` | Task 用来源类型和关联键幂等受理，API/event 请求摘要不可漂移，可选关联 Session；每个 Run 必须归属 Task。外部动作按 Task + 操作键唯一，参数摘要不可漂移，效果未知必须查询后收敛 |
...
| 成果 | `artifacts`、`artifact_versions` | 新增版本，不覆盖旧文件；明确关联 Session 或 Task；下载检查空间、会话共享规则或 Task 所有人权限 |
...
- [PF-01 Task 与外部操作基础](../../server/migrations/0050_task_operation_foundation.sql)、[无 Session 执行与 Task Artifact](../../server/migrations/0051_session_neutral_task_execution.sql)。
