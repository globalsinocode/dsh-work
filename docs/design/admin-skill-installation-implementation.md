...
交互已确认。管理助手中的 Skill 安装已接入 PostgreSQL、既有 Run/Attempt 调度器、Runtime Adapter 和 DSH；Agent 管理与运维对话尚未接入，ZIP 页仍为明确标记的交互预览。对话安装现在按“结构化计划 → 一次确认 → 原子草稿安装 → 严格试运行 → 管理员发布”执行。
...
- 包内容、文件摘要、来源与提交随安装记录保存；版本 manifest 保存不可变完整资源。员工执行和试运行只先收到 Skill 名称、说明与版本目录，调用 `activate_skill` 后才取得锁定正文和资源目录。旧 Manifest 保留正文直注的兼容读取，新生成的 Manifest 使用渐进式激活。
