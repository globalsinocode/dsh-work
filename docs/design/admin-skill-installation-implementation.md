...
对话确认成功时，同一事务还会写入一条确定性的助手结果消息，包含 Skill 名称、平台标识、0.1.0 待验证状态和发布前的下一步。消息 ID 与安装记录绑定，重复确认不会产生重复回复；迁移 0034 为历史成功安装补齐同样的结果消息。前端收到结果后滚动到最新回复，并优先显示成功状态与「前往 Skill 中心验证并发布」，完整安装计划收起供复核。
...
Skill 中心通过异步试运行接口立即取得 Run 标识，并按秒读取 Run、Runtime 调度、`activate_skill` 和 `python_execute` 的持久化证据。管理端弹窗只展示标准运行状态和一条简化结论，不重复展示 DSH 的完整回复，也不展示模型隐藏推理；严格试运行通过后，管理员在同一弹窗确认结果并发布。
