...
2. 配置创建：填写名称、说明、Skill/Tool 引用与权限，创建草稿版本；如确需 Agent 委派，只能选择精确的已发布目标 Agent Version，并配置最大深度、单父任务并行上限和单次超时。确认页及发布工作台必须展示目标数量和边界，未选择时明确显示未启用。服务端自动为草稿建立发布候选（默认五类试运行案例：成功、无效输入、权限拒绝、提示注入、能力特有失败）。
