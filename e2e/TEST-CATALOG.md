...
4. 使用匹配查询创建新 Attempt，确认只解析与当前用户、工作空间、角色和精确 Agent Version 同时匹配的当前版本；Manifest 固定版本和摘要，运行输出列出非权威记忆引用。
5. 令用户退出空间、撤销 Manifest 固定角色、增加新角色、改变 Agent Version 或升级记忆，确认角色 ACL 始终取当前角色与 Manifest 快照角色的交集，旧引用不能在领取或等待恢复时继续使用。新角色不能扩大旧 Attempt，新版本只影响新的 Attempt，不原地改写历史 Manifest。
6. 员工在用户中心撤回来源授权；确认新的检索返回空、已固定但尚未执行或等待恢复的 Attempt 被阻断，已完成运行的来源引用和审核审计仍可追溯。
7. 验证记忆提示明确标为非权威参考，不能覆盖系统指令、当前权限、工具结果或权威业务记录。

**验收：** 未经用户明确授权和管理员审核的内容不进入 Agent 上下文；检索与恢复均执行当前 ACL、来源、保留期和精确版本检查；撤回立即阻止未来使用但保留必要审计；记忆版本不可变，冲突通过新版本收敛；平台不建立第二套 Agent Loop，也不把知识库、业务台账、聊天历史或检查点伪装成记忆。
...
1. 使用真实 DSH 完成来源 Attempt、候选提交、审核和匹配查询，留存 Manifest 固定版本、运行输出引用与审计证据。
2. 分别变化员工身份、团队成员关系、角色和 Agent Version，确认跨用户、跨空间、跨角色和跨版本均不能检索或恢复旧记忆。
