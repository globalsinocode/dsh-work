...
- **P1 集成工作闭环**，有效员工 A 与 B，独立可丢弃 PostgreSQL + 受控身份、无真实模型：准备 A 的 60+ 轮旧会话、空会话、个人材料、会话附件、生成成果及一个已撤权团队。通过页面搜索并恢复旧对话；上传和引用文件；下载并逻辑移除；对话移除后成果保留、来源标记已移除。B 和失权团队数据不得返回。对应 `e2e/personal-workbench.spec.ts` 与服务器 `review-b3/b4/b5` 集成套件；页面的 API 模拟测试只证明交互，不代替这些真实 PostgreSQL 测试。
