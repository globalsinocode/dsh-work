...
- **P0 原型导航**，普通员工，内存合成资料：进入新对话、历史对话、我的文件、团队空间；能访问四个工作入口，个人空间不作为可管理卡片或选择器；旧个人链接解析到本人资源入口。AG-03 既有「自动任务」经决定保留为第五入口（不属于个人空间概念）。对应 `e2e/personal-workbench.spec.ts`。原型不承诺持久化或真实执行。
