...
3. 员工发送普通任务（仅回答、无可核验交付物）：执行终态仍为成功，页面显示「结果待核验」与不完整业务判据的缺口明细，不出现「目标已达成」；结果接口 `outcome` 为 `unverified`、`pendingItems` 含 `no_verified_deliverable`。
4. 员工发送含 `P1-成果缺口` 标记的任务：执行终态仍为成功，但页面显示「结果待核验」警示与成果登记缺口明细，不出现「目标已达成」；结果接口 `outcome` 为 `unverified`、回执含 `missing`。
5. 历史中由平台直接登记的旧 Run（无 Attempt 事件与回答登记）打开后同样按证据核验展示，不由 `succeeded` 推断达成。

**验收：** 执行状态与业务结果状态在 UI 上分列；模型自述不构成核验依据；缺登记证据的成功执行明确显示待核验；结果接口与详情同一授权边界。
...
**前置数据：** 健康的 DSH Runtime 内置工具连接器；Runtime 同时报告安全文件工具、`bash` 与未知工具；数据库另有非 DSH Connector 的工具记录
...
5. 查询普通工具列表与绑定列表，检查非 DSH 工具未混入；进入 MCP 连接器查看外部 MCP 能力。

**验收：** `edit` 可添加并锁定版本；`bash` 明确显示因任意 Shell/逐次审批能力未就绪而不可用；未知工具不可添加；客户端输入不能覆盖平台审批策略；被拒工具不能进入 Agent Runtime 授权闭包；`DSH 工具管理` 仅显示 `connector-dsh-workspace` 下的 DSH 内置工具，MCP 外部工具只在 MCP 连接器详情中出现。
...
**验收：** 两种入口产生同一候选/检查/试运行/发布流程与持久化数据；缺失依赖、包内候选、修订不一致均可复现地阻塞或放行；配置入口保存的委派策略随版本指纹和候选修订固定，ZIP 不能覆盖平台策略；提交→退回→重提交→发布往返与撤回全程留痕；证据随版本持久保存。
...
- **P0 原型导航**，普通员工，内存合成资料：主导航只显示新对话、团队空间和自动任务；历史对话与我的文件保留上下文入口及直达路由，不显示为主导航项。个人空间不作为可管理卡片或选择器，旧个人链接继续解析到本人资源入口。对应 `e2e/personal-workbench.spec.ts`。原型不承诺持久化或真实执行。
...
**spec：** `server/src/infrastructure/postgres/pf05-controlled-memory.integration.test.ts`、`server/src/modules/runtime/runtime-manifest-schema.test.ts`、`server/src/modules/runtime/runtime-adapter.test.ts`、`apps/admin-web/src/views/MemoryGovernanceView.test.ts`、员工端候选与撤回组件测试
