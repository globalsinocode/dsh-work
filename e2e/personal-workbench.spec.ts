...
test('P0: personal Workspace is implicit, navigation exposes four work entries', async ({ page }) => {
  await page.goto('/workbench')
  const navigation = page.getByRole('navigation', { name: '员工工作台主导航' })
  await expect(navigation.getByRole('button')).toHaveText(['新对话', '历史对话', '我的文件', '团队空间'])
