...
describe('global management conversation history', () => {
  it('restores history from another page using the standard collapsible navigation group', async () => {
    ...
    const devAccess = navigation.findAll('section').find(section => section.text().includes('开发接入'))
    const securityOps = navigation.findAll('section').find(section => section.text().includes('安全与运维'))
    expect(devAccess?.text() ?? '').toContain('接入规范')
    expect(devAccess?.text() ?? '').toContain('接口文档')
    expect(securityOps?.text() ?? '').toContain('Runtimes')
