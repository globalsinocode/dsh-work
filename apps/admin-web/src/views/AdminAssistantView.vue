          ...
          <div v-else class="assistant-messages" role="log" aria-label="管理对话记录" aria-live="polite">
            <div v-for="run in assistant.current.runs" :key="run.id" class="assistant-run">
              ...
              <div v-if="installationFor(run.id)" class="action-card">
                ...
                <SkillPackagePreview v-if="installationFor(run.id)!.package" :source="installationFor(run.id)!.resolvedUrl ?? installationFor(run.id)!.source" :package="installationFor(run.id)!.package!" :resolved-ref="installationFor(run.id)!.resolvedRef ?? undefined" />
                <footer v-if="installationFor(run.id)!.status === 'pending' && auth.canManage"><span>请确认包内容、来源摘要及工具权限</span><el-button :disabled="busy" @click="assistant.cancel(run.id)">取消安装</el-button><el-button type="primary" :loading="busy" :disabled="run.status !== 'succeeded' || !installationFor(run.id)!.package" @click="assistant.confirm(run.id, installationFor(run.id)!.package!.sha256)">确认安装</el-button></footer>
