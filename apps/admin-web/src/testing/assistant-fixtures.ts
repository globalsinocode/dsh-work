...
export function conversationFixture(): AdminConversation {
  return { id: 'admin-session-00000000-0000-4000-8000-000000000001', title: '安装 Skill',
    ...
    installations: [{ id: 'install-1', runId: 'run-1', source: 'https://example.org/skill.zip', resolvedUrl: null, resolvedRef: null, status: 'pending', skillId: null,
      package: { name: '真实测试包', description: '来自服务端的包说明', version: null, sha256: 'a'.repeat(64), archiveSha256: 'b'.repeat(64), instructions: '按包内规则执行，不生成新的指令或数据。', toolIds: [], files: [{ path: 'SKILL.md', size: 150, sha256: 'c'.repeat(64) }] },
    }],
  }
}
